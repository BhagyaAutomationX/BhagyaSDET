<div align="center">

# ⚡ BHAGYASHRI SHASTRAKAR ⚡

<img src="https://readme-typing-svg.herokuapp.com?font=Orbitron&size=32&duration=2500&color=00F7FF&center=true&vCenter=true&width=900&lines=SOFTWARE+QA+ENGINEER;AUTOMATION+TESTING+SPECIALIST;SELENIUM+%7C+JAVA+%7C+TESTNG;REST+ASSURED+%7C+API+TESTING;AI+TOOLS+%7C+GENERATIVE+AI" />

<img src="https://user-images.githubusercontent.com/74038190/212747919-84b68444-0d81-46db-a338-7ec50f4de3e2.gif" width="100%">

</div>

---

# 💫 ABOUT ME

<img align="right" width="380" src="https://media.giphy.com/media/L8K62iTDkzGX6/giphy.gif">

```yaml
Name: Bhagyashri Shastrakar
Role: Software QA Engineer
Location: India
Focus: Automation Testing & AI
Experience: Manual + Automation Testing
Passion: Building Scalable Automation Frameworks
```

🚀 Passionate QA Engineer skilled in building enterprise-level automation frameworks using Selenium WebDriver, Java, TestNG, Maven, Jenkins, and REST Assured.

🔥 Experienced in validating real-time business workflows including Customer, Product, Payment, and Order Management systems.

⚡ Strong understanding of:

* SDLC & STLC
* Agile Scrum Methodology
* Defect Lifecycle
* Cross Browser Testing
* Functional & Regression Testing
* API Testing
* Database Validation

🧠 Currently learning:

* Advanced Automation Framework Design
* CI/CD with Jenkins
* Python Automation
* AI Tools & Prompt Engineering
* Generative AI

---

# 🛠 TECH STACK

<div align="center">

<img src="https://skillicons.dev/icons?i=java,python,mysql,git,github,maven,jenkins,eclipse,vscode" />

</div>

---

# ⚙ AUTOMATION EXPERTISE

<table>
<tr>
<td width="50%">

## ✅ Automation Testing

* Selenium WebDriver
* TestNG
* POM Framework
* Data Driven Testing
* Hybrid Framework
* Cross Browser Testing
* XPath & CSS Selectors
* Wait Mechanisms

</td>

<td width="50%">

## ✅ API & Database

* REST Assured
* Postman
* JSON Validation
* CRUD Operations
* MySQL Queries
* API Automation
* Response Validation
* Authentication Testing

</td>
</tr>
</table>

---

# 🚀 FEATURED PROJECTS

## 🔥 Adaptive Retail Quality Engine

```diff
+ Enterprise-Level Automation Framework
+ Selenium + Java + TestNG
+ Reporting + Listeners + Screenshots
+ Scalable Framework Architecture
+ Maven Integrated
```

---

## 🔥 Selenium Java Framework

```diff
+ POM Design Pattern
+ Data Driven Testing
+ Reusable Utilities
+ Advanced XPath Strategies
+ Real-Time Framework Structure
```

---

## 🔥 REST Assured Advanced Framework

```diff
+ API Automation Framework
+ CRUD API Testing
+ JSON Schema Validation
+ Authentication Handling
+ Maven + Reporting Integration
```

---

# 📊 GITHUB ANALYTICS

<div align="center">

<img height="170" src="https://github-readme-stats.vercel.app/api?username=BhagyaQA-S&show_icons=true&theme=radical&hide_border=true&count_private=true"/>

<img height="170" src="https://github-readme-stats.vercel.app/api/top-langs/?username=BhagyaQA-S&layout=compact&theme=radical&hide_border=true"/>

</div>

---

# 🔥 CONTRIBUTION STREAK

<div align="center">

<img src="https://streak-stats.demolab.com?user=BhagyaQA-S&theme=radical&hide_border=true"/>

</div>

---

# 🏆 ACHIEVEMENTS

🏅 Built multiple Selenium Automation Frameworks

🏅 Practicing enterprise-level QA workflows

🏅 Learning AI + Automation integration

🏅 Building scalable API Automation projects

🏅 Consistently improving coding & testing skills

---

# 🌐 CONNECT WITH ME

<div align="center">

<a href="https://github.com/BhagyaAutomationX/BhagyaSDET">
<img src="https://img.shields.io/badge/GitHub-000?style=for-the-badge&logo=github&logoColor=white"/>
</a>

<a href="www.linkedin.com/in/bhagya-shastrakar-a7956740b">
<img src="https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white"/>
</a>

<a href="mailto:bhagyashastrakar1@gmail.com">
<img src="https://img.shields.io/badge/Gmail-D14836?style=for-the-badge&logo=gmail&logoColor=white"/>
</a>

</div>

---

# 👨‍💻 PORTFOLIO

📌 GitHub Repositories
👉 https://github.com/BhagyaAutomationX/BhagyaSDET

📌 Resume
👉 https://drive.google.com/file/d/11jg7VYToy9tnnt1B2GNA5VThQgERE8eg/view?usp=drivesdk

---

# ⚡ QA ENGINEER QUOTE

<div align="center">

## ““Software QA is not about breaking applications —

## it is about building trust before users ever touch the product.””

</div>

---

<div align="center">

<img src="https://komarev.com/ghpvc/?username=BhagyaQA-S&label=PROFILE+VIEWS&color=blueviolet&style=for-the-badge"/>

<img src="https://capsule-render.vercel.app/api?type=waving&color=0:00F7FF,100:8A2BE2&height=120&section=footer"/>

</div>
